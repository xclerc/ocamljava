...
let global_symbol id =
  let sym = Jcompilenv.symbol_for_global id in
  try ndl_loadsym sym
  with _ -> fatal_error ("Javatoploop.global_symbol " ^ (Ident.unique_name id))
  ...
  match (try Result (Obj.magic (ndl_run_toplevel dll entry)) with exn -> Exception exn) with
    ...
    | Result r ->
        match Obj.magic r with
          | Ok x -> Result x
          | Err s -> fatal_error ("Javatoploop.dll_run " ^ s)
...
let rec eval_path = function
  ...
  | Papply(_p1, _p2) ->
      fatal_error "Javatoploop.eval_path"
...
let load_lambda ppf (size, lam) =
  ...
  Javagen.compile_implementation fn ppf (size, slam);
...
let rec pr_item env items =
  ...
  | Sig_modtype(id, decl) :: rem ->
      ...
      Some (tree, None, rem)
  | Sig_class(id, decl, rs) :: _cltydecl :: _tydecl1 :: _tydecl2 :: rem ->
      let tree = Printtyp.tree_of_class_declaration id decl rs in
      Some (tree, None, rem)
  | Sig_class_type(id, decl, rs) :: _tydecl1 :: _tydecl2 :: rem ->
  ...
  try
    ...
    let success =
      protect Location.input_name filename (fun () ->
        ...
        | x -> Javaerrors.report_error ppf x; false) in
...
let set_paths () =
  ...
  load_path := !load_path @ [Filename.concat Config.standard_library "javalib"];
...
let loop ppf =
  fprintf ppf "        OCaml version %s - Java toplevel@.@." Config.version;
  ...
  while true do
    ...
    | PPerror -> ()
    | x -> Javaerrors.report_error ppf x; Btype.backtrack snap
  done
...
let run_script ppf name args =
  ...
  if Array.length Sys.argv < len then invalid_arg "Javatoploop.run_script";
